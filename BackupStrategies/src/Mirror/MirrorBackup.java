...
public class MirrorBackup implements BackupStrategy {
    ...
    private long getPathSize(Path srcPath) {
        ...
        class Visitor extends SimpleFileVisitor<Path> {
            ...
            public FileVisitResult visitFileFailed(Path file, IOException exc) {
                // Cannot get size of file strategy:
    ...
    private void pathExceptionStrategy(IOException exception, SimplePair<Path> paths) {
        // Logging exception occurred while failed tried of executing specified path set:
    ...
    private Statistics backup(Path sourcePath, Path destinationPath,
        ...
        FileVisitor<? super Path> fileCopyVisitor = new SimpleFileVisitor<>() {
            ...
            public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                ...
                // Interrupt backup check:
                if (isInterrupted.get()) { return FileVisitResult.TERMINATE; }
                // Defining path of current directory in destination backup location:
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                ...
                // Interrupt backup check:
                if (isInterrupted.get()) { return FileVisitResult.TERMINATE; }
                // Defining path of current file in destination backup location:
                Path dstFile = resolvedDestinationPath.resolve(sourcePath.relativize(file));
                // Decision of copying file:
                if (!(Files.isRegularFile(dstFile) && comparisonStrategy.apply(file, dstFile))) {
                    try {
                        // Copying file:
                ...
                try {
                    // Adding path size to global files size sum:
                    pathSizeSum.add(Files.size(file));
                } catch (IOException exc) {
                    // Only logging error:
                    logger.log(Level.WARNING, "Getting file size: %s".formatted(file), exc);
                }
                // Sending new progress of copied files:
                setProgress((int) (pathSizeSum.sum() * 100 / (fileSizes == 0 ? 1 : fileSizes)));
                return FileVisitResult.CONTINUE;
            }
        ...
        FileVisitor<? super Path> fileRemoveVisitor = new SimpleFileVisitor<>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                // Interrupt backup check:
                if (isInterrupted.get()) { return FileVisitResult.TERMINATE; }
                // Checking if path of current file exists in source backup location:
                else if (!Files.isRegularFile(sourcePath.resolve(resolvedDestinationPath.relativize(file)))) {
                    try {
            ...
            public FileVisitResult postVisitDirectory(Path dir, IOException exc) throws IOException {
                ...
                // Checking if path of current directory exists in source backup location:
        ...
        // Variable which stores information if there is a need to check for additional files in destination path:
        boolean isPureBackup = false;
        try {
            // Creating directory of source path last directory name:
            if (!Files.isDirectory(resolvedDestinationPath)) {
                isPureBackup = true;
                try {
    ...
    // Strategies change state of properties:
